...
void STEPPERMOTOR_voidRotateWithAngle(uint8 Copy_u8Direction,uint8 Copy_u8Angle)
{
	...
	if( Copy_u8Direction == ROTATE_CLOCKWISE )
	{

		for(Local_u16Counter_I = 0 ; Local_u16Counter_I < Local_u16Stepper_Full_Step / 4 ;Local_u16Counter_I++)
		{
			STEPPERMOTOR_voidRotateClockWise();
		}

	}else if (Copy_u8Direction == ROTATE_COUNTERCLOCKWISE)
	{
		for(Local_u16Counter_I = 0 ; Local_u16Counter_I < Local_u16Stepper_Full_Step / 4 ;Local_u16Counter_I++)
		{
			STEPPERMOTOR_voidRotateCounterClockWise();
		}
